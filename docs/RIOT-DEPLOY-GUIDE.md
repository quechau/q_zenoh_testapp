...
| a writable point flaps **ok ↔ fault** on every value update | the device is **rejecting the written value** — e.g. a temperature (26) wired into a 0/1 command register; each rejected write faults the point, each successful poll clears it | check the register's semantics; put a compare/threshold between an analog source and a command register (measured: edge `edgeLoraTemp.out → hvac-power.in`) |
